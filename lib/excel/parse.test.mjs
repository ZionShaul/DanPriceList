...
test("שם גנרי נקלט כשם תקני; '-' נחשב ריק", () => {
  const headers = ["שם משק", "תאור פריט", "כמות", "מחיר", "סכום", "שם גנרי"];
  const hmap = buildHeaderMap(headers);
  assert.equal(hmap.generic, "שם גנרי");
  const recs = [
    { "שם משק": "משק א", "תאור פריט": "אוראן 32% M", "כמות": 1, "מחיר": 10, "סכום": 10, "שם גנרי": "אוראן 32%" },
    { "שם משק": "משק א", "תאור פריט": "מוצר ללא גנרי", "כמות": 1, "מחיר": 10, "סכום": 10, "שם גנרי": "-" },
  ];
  const res = classifyRows(recs, hmap);
  assert.equal(res.valid[0].generic, "אוראן 32%");
  assert.equal(res.valid[1].generic, null);
});
